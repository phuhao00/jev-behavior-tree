...
namespace jev {
namespace {
Pairs pairs(std::initializer_list<std::pair<const char*, const char*>> items) {
    Pairs out;
    for (auto [k, v] : items) out.emplace_back(k, v);
    return out;
}
std::vector<std::string> lines(std::initializer_list<const char*> items) { return {items.begin(), items.end()}; }
}
...
std::optional<Pairs> tactics_for_role(const std::string& role) {
    ...
    if (role == "companion") return pairs({
...
std::optional<std::pair<std::string, std::string>> prior_for_role(const std::string& role) {
    if (role == "guard") return std::pair<std::string, std::string>{"Posted to this spot. Leaves the post only for a real threat or a fallen ally.", "Keep this place from being taken."};
    if (role == "civilian") return std::pair<std::string, std::string>{"Wants to live through the next minute. Will not play the hero.", "Survive. Run or hide before fighting."};
    if (role == "predator") return std::pair<std::string, std::string>{"Hunts. Prefers a sure kill over a fair fight, and leaves when wounded.", "Feed, then leave."};
    if (role == "companion") return std::pair<std::string, std::string>{"Stays with the player. Will step into a hit for them, and will not abandon a fallen ally.", "Keep the player alive."};
    if (role == "ambient") return std::pair<std::string, std::string>{"A piece of the place. It reacts, then settles. It does not chase.", "Belong to the scene."};
    return std::nullopt;
}
...
const char* opening_kind_for(const std::string& role) {
    ...
    if (role == "guard" || role == "predator") return "ambush";
    return "generic";
}

std::pair<std::string, Pairs> opening_question(const std::string& role) {
    if (role == "civilian" || role == "ambient") return {"Is this a good moment for this agent to slip away or go still without being noticed? Use `scene`, `player`, and `nearby`.", pairs({{"true", "Attention is elsewhere, cover exists, or the threat is busy with someone else."}, {"false", "This agent is already seen, cornered, or nobody is pursuing them."}})};
    if (role == "companion") return {"Is this a good moment to scout, intercept, or step in for the player, rather than keep trailing? Use `player` and `nearby`.", pairs({{"true", "A threat is forming that the player has not answered, or an ally is about to drop."}, {"false", "Staying on the current follow or fight is the useful thing."}})};
    if (role == "guard" || role == "predator") return {"Is this a good moment to strike or confront first, rather than wait? Use cover, surprise, `player` activity, and `nearby`.", pairs({{"true", "The target is exposed, distracted, or already committing, and waiting would waste it."}, {"false", "The target is ready, too far, or this agent would be striking into a prepared defense."}})};
    return {"Is there a fleeting opening this agent should take right now, given `agent.goal`?", pairs({{"true", "Waiting would waste a real opening."}, {"false", "Nothing is opening, or acting now is worse than waiting."}})};
}

nlohmann::json pairs_json(const Pairs& items) {
