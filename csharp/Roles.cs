...
{
    public const string HoldTactic = "hold";
    public const string HoldDirective = "hold_atmosphere";

    public static IReadOnlyList<string> ThreatLevels() =>
    [
    ...
    public static IReadOnlyList<string> TensionLevels() =>
    [
    ...
    public static List<(string Key, string Text)> WorldDirectives() => Pairs(
    ...
    public static List<(string Key, string Text)>? TacticsForRole(string role) => role switch
    {
        "guard" => Pairs(
    ...
    public static (string Personality, string Goal)? PriorForRole(string role) => role switch
    {
        "guard" => ("Posted to this spot. Leaves the post only for a real threat or a fallen ally.", "Keep this place from being taken."),
        "civilian" => ("Wants to live through the next minute. Will not play the hero.", "Survive. Run or hide before fighting."),
        "predator" => ("Hunts. Prefers a sure kill over a fair fight, and leaves when wounded.", "Feed, then leave."),
        "companion" => ("Stays with the player. Will step into a hit for them, and will not abandon a fallen ally.", "Keep the player alive."),
        "ambient" => ("A piece of the place. It reacts, then settles. It does not chase.", "Belong to the scene."),
        _ => null,
    };

    public static string OpeningKindFor(string role) => role switch
    {
        "civilian" or "ambient" => "escape",
        "companion" => "scout",
        "guard" or "predator" => "ambush",
        _ => "generic",
    };

    public static (string Instructions, List<(string Key, string Text)> Criteria) OpeningQuestion(string role) => role switch
    {
        "civilian" or "ambient" => ("Is this a good moment for this agent to slip away or go still without being noticed? Use `scene`, `player`, and `nearby`.", Pairs(
        ...
        "companion" => ("Is this a good moment to scout, intercept, or step in for the player, rather than keep trailing? Use `player` and `nearby`.", Pairs(
        ...
        "guard" or "predator" => ("Is this a good moment to strike or confront first, rather than wait? Use cover, surprise, `player` activity, and `nearby`.", Pairs(
        ...
        _ => ("Is there a fleeting opening this agent should take right now, given `agent.goal`?", Pairs(
            ("true", "Waiting would waste a real opening."),
            ("false", "Nothing is opening, or acting now is worse than waiting."))),
    };

    public static JsonObject ToObject(IEnumerable<(string Key, string Text)> pairs)
    {
        var obj = new JsonObject();
        foreach (var (key, text) in pairs) obj[key] = text;
        return obj;
    }

    private static List<(string Key, string Text)> Pairs(params (string, string)[] items) => items.ToList();
}
