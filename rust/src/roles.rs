pub const HOLD_TACTIC: &str = "hold";
pub const HOLD_DIRECTIVE: &str = "hold_atmosphere";
...
pub fn threat_levels() -> Vec<String> {
    lines(&[
...
pub fn tension_levels() -> Vec<String> {
    lines(&[
...
pub fn world_directives() -> Vec<(String, String)> {
    pairs(&[
...
pub fn tactics_for_role(role: &str) -> Option<Vec<(String, String)>> {
    let items: &[(&str, &str)] = match role {
        "guard" => &[
...
pub fn prior_for_role(role: &str) -> Option<(&'static str, &'static str)> {
    match role {
        "guard" => Some(("Posted to this spot. Leaves the post only for a real threat or a fallen ally.", "Keep this place from being taken.")),
        "civilian" => Some(("Wants to live through the next minute. Will not play the hero.", "Survive. Run or hide before fighting.")),
        "predator" => Some(("Hunts. Prefers a sure kill over a fair fight, and leaves when wounded.", "Feed, then leave.")),
        "companion" => Some(("Stays with the player. Will step into a hit for them, and will not abandon a fallen ally.", "Keep the player alive.")),
        "ambient" => Some(("A piece of the place. It reacts, then settles. It does not chase.", "Belong to the scene.")),
        _ => None,
    }
}

pub fn opening_kind_for(role: &str) -> &'static str {
    match role {
        "civilian" | "ambient" => "escape",
        "companion" => "scout",
        "guard" | "predator" => "ambush",
        _ => "generic",
    }
}

pub fn opening_question(role: &str) -> (&'static str, Vec<(String, String)>) {
    match role {
        "civilian" | "ambient" => (
